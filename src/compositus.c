/*
 * compositus.c - Main application core
 *
...
static void dump_session_state(void)
{
	...
	for (i = 0; i < nres; i++) {
		...
		fprintf(debug_log, "\tsession_id   : %s\n",
				tcmapget2(cols, "session_id"));
		fprintf(debug_log, "\tcsrf_token   : %s\n",
				tcmapget2(cols, "csrf_token"));
...
static void create_server(int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		pid = fork();
		if (pid == 0) {  /* child */
			set_proc_title("compositus: worker");
			accept_request();
		}
	}
...
int main(int argc, char **argv)
{
	...
	set_proc_title("compositus: master");
